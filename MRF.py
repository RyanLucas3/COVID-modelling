...
class MacroRandomForest:
    ...
    def _array_setup(self):
        ...
        self.pred_kf = np.stack(
            [np.tile(np.nan, (len(self.data), len(self.x_pos)+1))]*(self.B))
    ...
    def _ensemble_loop(self):
        ...
        for b in Bs:
            ...
            if self.VI_rep > 0:
                not_in_rando_vec = [i for i in range(
                    self.pred_kf.shape[2]) if i not in rando_vec]

                self.pred_kf[b, not_in_rando_vec,
                             :] = rt_output['fitted_shu'][not_in_rando_vec, :]
        ...
        if self.oos_flag:
            ...
            # self.data = self.data.iloc[-self.fake_pos, :]
        ...
        ###################################################################################
        ###################################################################################
        first = pd.DataFrame(self.pred_kf[0]).notna()
        display(first[first == True])
        if not self.random_z:
            if self.VI_rep > 0:
                if self.bootstrap_opt != 0:

                    oob_pred_shu = self.pred_kf[0]

                    for b1 in range(self.pred_kf.shape[0]):
                        oob_pred_shu += self.pred_kf[b1]

                    oob_pred_shu /= self.B

                    oob_crit_shu = np.repeat(
                        np.nan, repeats=len(self.x_pos) + 1, axis=0)

                    normalized_target = np.matrix(
                        scale(self.data.iloc[:, self.y_pos])).T

                    for k in range(0, len(self.x_pos)):

                        not_in_oos = [i for i in range(
                            len(self.data)) if i not in self.oos_pos]

                        oob_crit_shu[k] = np.sqrt(np.nanmean(
                            (oob_pred_shu[not_in_oos, k]-normalized_target[not_in_oos, :])**2))  # full-blown model

                        VI_oob = oob_crit_shu/oob_crit_shu[0]-1

                        which_oob = self._which_min_n(-1 *
                                                      VI_oob[0:len(self.x_pos)], self.keep_VI)

                        VI_oob = VI_oob[1:]

                        which_all = np.unique(list(which_oob))
                ...
                if len(self.oos_pos) > 0 and not self.oos_flag:

                    poos_crit_shu = np.repeat(
                        np.nan, repeats=len(self.x_pos) + 1, axis=0)

                    for k in range(0, len(self.x_pos)):
                        poos_crit_shu[k] = np.sqrt(np.nanmean(
                            (oob_pred_shu[self.oos_pos, k]-normalized_target[self.oos_pos])**2))  # full-blown model

                    VI_poos = poos_crit_shu/poos_crit_shu[0] - 1
                    VI_poos = VI_poos[1:]

                    self.keep_VI = min([self.keep_VI, len(VI_poos)])
                    which_poos = self._which_min_n(-1 *
                                                   VI_poos[0:len(self.x_pos)], self.keep_VI)
                ...
                if not self.oos_flag:
                    which_all = np.unique(list(which_poos) + list(which_oob))

                beta_shu_crit = np.tile(
                    np.nan, (len(self.z_pos)+1, len(self.x_pos)+1))

                for jj in range(0, len(self.z_pos)):
                    for k in range(1, len(self.z_pos)-1):
                        beta_shu_crit[jj, k] = np.sqrt(np.nanmean(
                            (self.betas_shu_nonOVF[:, jj, k]-self.betas_shu_nonOVF[:, jj, 1])**2))  # full-blown model

                VI_betas_nonOVF = beta_shu_crit[:, 1:]

                for jj in range(0, len(self.z_pos)):

                    which_betas = self._which_min_n(
                        -VI_betas_nonOVF[jj, :], max(math.floor(self.keep_VI/2), 1))
                    which_all = np.unique(list(which_all) + list(which_betas))

                impZ = self.data.iloc[:, self.x_pos[which_all]]
    ...
    def _one_MRF_tree(self, b):
        ...
        beta_bank_shu, fitted_shu = self._variable_importance(
            leafs, fitted, fitted_scaled, beta_bank, rando_vec, rw_regul_dat)
    ...
    def _variable_importance(self, leafs, fitted, fitted_scaled, beta_bank, rando_vec, rw_regul_dat):
        '''
        ...
        if self.VI_rep > 0:
            ...
            for k in range(len(self.x_pos)):
                if whos_in[k]:
                    for ii in range(1, self.VI_rep+1):
                    ...
                    beta_bank_shu[k+1] = beta_bank
                    fitted_shu[:, k+1] = fitted_scaled

            beta_bank_shu[0] = beta_bank
            fitted_shu[:, 0] = fitted_scaled
    ...
    def _which_min_n(self, x, n):
        if n == 1:
            ...
            if n > 1:
                ...
                ii = sorted(range(len(x)), key=lambda k: x[k])[
                    0: min(n, len(x))]

                ii = [i for i in ii if not math.isnan(x[i])]
                return ii
            else:
                return None
